  ...
  return (
    <Section spacing="lg">
      <Container size="narrow">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8 no-print pb-6 border-b border-zinc-200 dark:border-zinc-900">
          <div>
        ...
        <div className="bg-white/90 dark:bg-zinc-950/80 sm:border border-zinc-200 dark:border-zinc-800/80 shadow-md dark:shadow-none rounded-2xl p-6 sm:p-12 space-y-10">
          {/* Header */}
          <div className="border-b border-zinc-200 dark:border-zinc-800/80 pb-8">
            <Heading as="h1" className="text-3xl sm:text-4xl text-zinc-900 dark:text-zinc-100 font-extrabold mb-2">
              {profile.personal.name}
            </Heading>
            <div className="text-lg font-mono text-emerald-600 dark:text-emerald-400 mb-4">
              {getLocalized(profile.personal.headline, lang)}
            </div>

            <div className="flex flex-wrap gap-y-2 gap-x-6 text-xs font-mono text-zinc-600 dark:text-zinc-400">
          ...
          <div>
            ...
            <Text variant="body" className="text-zinc-700 dark:text-zinc-300 leading-relaxed text-sm sm:text-base">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {skills.map((s) => (
                <div key={s.id} className="p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/60">
          ...
          <div>
            ...
            <div className="space-y-8">
              {experiences.map((exp) => (
                <div key={exp.id} className="border-l border-zinc-200 dark:border-zinc-800 pl-4 space-y-2">
                  ...
                  <Text variant="body" className="text-zinc-700 dark:text-zinc-300 text-sm">
                    {getLocalized(exp.summary, lang)}
                  </Text>

                  {exp.highlights && exp.highlights.length > 0 && (
                    <ul className="mt-2 space-y-1.5 pl-2 text-xs text-zinc-700 dark:text-zinc-300">
                  ...
                  {exp.engineeringCases && exp.engineeringCases.length > 0 && (
                    <div className="mt-3 space-y-2 pt-2 border-t border-zinc-200 dark:border-zinc-900">
                      {exp.engineeringCases.map((ec, idx) => (
                        <div key={idx} className="text-xs text-zinc-600 dark:text-zinc-400 space-y-1">
                          <span className="font-semibold text-zinc-800 dark:text-zinc-200 block">
                            • {getLocalized(ec.title, lang)}:
                          </span>
                          <span className="text-zinc-700 dark:text-zinc-300 block pl-3">
